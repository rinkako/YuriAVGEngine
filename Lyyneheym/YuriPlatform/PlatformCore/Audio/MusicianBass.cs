...
namespace Yuri.PlatformCore.Audio
{
    ...
    internal class MusicianBass
    {
        ...
        public void RePerform(MusicianDescriptor mdescriptor)
        {
            ...
            else if (mdescriptor.PlayingBGM == this.BgmHandleContainer.Key)
            {
                this.SetBGMVolume(mdescriptor.BGMVol * (float)this.bgmVolume);
            }
            else
            {
                var c = ResourceManager.GetInstance().GetBGMGCHandle(mdescriptor.PlayingBGM);
                this.PlayBGM(mdescriptor.PlayingBGM, c.Key, c.Value, mdescriptor.BGMVol * (float) this.bgmVolume);
            }
            ...
            // 重演绎背景音效
                ...
                //    {
                //        this.SetBGSVolume(mdescriptor.BGSVol[i] * (float)this.bgsVolume, i);
                //    }
                //    else
                //    {
                //        this.PlayBGS(mdescriptor.PlayingBGS[i],
                //            ResourceManager.GetInstance().GetBGS(mdescriptor.PlayingBGS[i]),
                //            mdescriptor.BGSVol[i] * (float)this.bgsVolume, i);
                //    }
                //}
            }
        ...
        {
            if (track >= 0 && track < GlobalConfigContext.GAME_MUSIC_BGSTRACKNUM && gch != null)
            {
